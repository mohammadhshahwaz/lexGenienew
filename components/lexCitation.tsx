...
//   const [blobUrl, setBlobUrl] = useState<string | any>(null);
//   const [viewUrl, setViewUrl] = useState<string>(""); // Add viewUrl state
//   const [loading, setLoading] = useState<boolean>(false);
//   const [error, setError] = useState<string | null>(null);
//   const [userMessage, setUserMessage] = useState(""); // New state for user message
//   const [chatHistory, setChatHistory] = useState<any[]>([]); // State for chat history
//   const [sessionID, setSessionID] = useState<string | null>(null); // Session ID for API calls
//   const [isResponseLoading, setIsResponseLoading] = useState<boolean>(false); // State for current response loading
//   const [history, setHistory] = useState<FileInfo[]>([]); // State for history
...
//       const response = await fetch(`${apiUrl}/legal-research/`, {
//         method: "POST",
//         body: formData,
//       });
...
//         text: data.research_result || "Generated lex Citation will appear here.",
//         sender: "bot",
//         timestamp: new Date().toLocaleString(),
//       };

//       setChatHistory([initialBotMessage]); // Initialize the chat with the first response
//       setSessionID(data.session_id); // Save the session ID for future interaction
...
//   const formatResponseWithRegex = (response: string) => {
//     return response
//       .split(/(?:\d+\.\s+|\•\s+|\-\s+|\*\s+)/) // Split by numbers or bullet points
//       .filter((line) => line.trim() !== "") // Remove empty lines
//       .map((line, index) => `${index + 1}. ${line.trim().replace(/\*/g, '')}`) // Remove asterisks and number the lines
//       .join("\n"); // Join lines with new lines for display
//   };
...
//     setIsResponseLoading(true); // Show loading for current response

//     // Call backend API for user interaction
//     try {
//       const response = await axios.post(
//         `${apiUrl}/continue-conversation-legal-research/`,
//         {
//           user_input: userMessage,
//           session_id: sessionID, // Use the session ID here
//         }
//       );

//       const formattedResponse = formatResponseWithRegex(response.data.response);

//       const botResponse = {
//         text: formattedResponse, // Use the formatted response directly
//         sender: "bot",
//         timestamp: new Date().toLocaleString(),
//       };

//       // Update chat history with the bot response without refreshing previous messages
//       setChatHistory((prev) => [...prev, botResponse]);
//     } catch (error) {
//       console.error("Error sending chat:", error);
//     } finally {
//       setIsResponseLoading(false); // Stop loading after response
//     }
//   };
